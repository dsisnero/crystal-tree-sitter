...
module TreeSitter
  ...
  #
  # `Language` instances are immutable after creation and safe to share across fibers/threads.
  class Language
    ...
    # :nodoc:
    # Languages are process-lifetime singletons in this binding (cached by pointer
    # in `Language.loaded_languages`). For native (non-WASM) grammars
    # `ts_language_delete` is a no-op, so this is informational.
    def finalize
      LibTreeSitter.ts_language_delete(to_unsafe)
    end
